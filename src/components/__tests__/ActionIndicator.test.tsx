...
import { createMockGameContext, createMockPlayer, createCards } from '../../test/test-utils'
...
describe('ActionIndicator', () => {
  ...
  it('shows game starting message', () => {
    ...
    expect(screen.getByText(/Dealing cards and starting the game/)).toBeInTheDocument()
  })

  it('shows current player turn message', () => {
    const context = createMockGameContext({
      players: [
        createMockPlayer({ id: 'player-1', name: 'Alice', isCurrentPlayer: true }),
        createMockPlayer({ id: 'player-2', name: 'Bob', isCurrentPlayer: false }),
      ],
  ...
  it('shows auto-play message for single valid card', () => {
    ...
    expect(screen.getByText(/Auto-playing Alice's only valid card/)).toBeInTheDocument()
  })
  ...
  it('shows auto-skip message for no valid cards', () => {
    ...
    expect(screen.getByText(/Alice has no valid moves - skipping turn/)).toBeInTheDocument()
  })
  ...
  it('shows card selection message when cards are selected', () => {
    ...
    expect(screen.getByText(/Press SPACE or click Play button/)).toBeInTheDocument()
  })
  ...
  it('shows waiting for turn message', () => {
    ...
    expect(screen.getByText(/Advancing to next player/)).toBeInTheDocument()
  })
  ...
  describe('game ending messages', () => {
    it('shows timer expired message', () => {
      ...
      expect(screen.getByText(/Time's up! Game ended after 3 minutes/)).toBeInTheDocument()
    })
    ...
    it('shows no valid moves message', () => {
      ...
      expect(screen.getByText(/No valid moves available for any player/)).toBeInTheDocument()
    })
    ...
    it('shows manual end message', () => {
      ...
      expect(screen.getByText(/Game manually ended by player/)).toBeInTheDocument()
    })
    ...
    it('shows player won message', () => {
      ...
      expect(screen.getByText(/A player emptied their hand and won/)).toBeInTheDocument()
    })
    ...
    it('shows default ending message for unknown reason', () => {
      ...
      expect(screen.getByText(/Game ended. Calculating final scores/)).toBeInTheDocument()
    })
  })
  ...
  it('shows game over message', () => {
    ...
    expect(screen.getByText(/Game completed! Check the results above/)).toBeInTheDocument()
  })
  ...
  it('applies correct styling classes', () => {
    const context = createMockGameContext()
    const { container } = render(<ActionIndicator context={context} currentState="lobby" />)
    
    const indicator = container.firstChild as HTMLElement
    expect(indicator).toHaveClass('flex', 'items-center', 'gap-3', 'px-4', 'py-3', 'rounded-lg', 'border-2')
  })
  ...
  it('displays appropriate icons for different states', () => {
    ...
    states.forEach(({ state, expectedIcon }) => {
      const context = createMockGameContext()
      const { unmount } = render(<ActionIndicator context={context} currentState={state} />)
